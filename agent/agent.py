...
class ResearchAgent:
    def __init__(self, memory: SessionMemory | None = None, run_id: str | None = None) -> None:
    ...
    def run(self, question: str) -> str:
        ...
        for _ in range(MAX_ITERATIONS):
            ...
            for tool_call in tool_calls:
                ...
                start = log_pre(tool_call.function.name, args, run_id=self.run_id)
                result = (
                    tool_fn(args)
                    if tool_fn is not None
                    else f"Error: unknown tool '{tool_call.function.name}'"
                )
                log_post(tool_call.function.name, result, start, run_id=self.run_id)
                messages.append(
                    {
                        "role": "tool",
